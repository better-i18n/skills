...
| Topic | Reference |
|---|---|
| **Next.js SDK** — App Router, ISR, middleware, locale switcher | `sdk-next.md` |
| **React / Core / Hono / Remix** — factory, TanStack Router, SSR | `sdk-react.md` |
| **Mobile** — Expo, Swift SPM, Flutter | `sdk-mobile.md` |
| **CLI** — scan, sync, check, doctor, CI gates | `cli.md` |
| **MCP tools** — all 13 translation tools, workflow order, safety | `mcp.md` |
| **Content CMS** — SDK query builder, 19 MCP content tools | `content.md` |
| **CDN** — URL structure, cache layers, purge flow, fallback | `cdn.md` |
| **GitHub sync** — pipeline, job types, publish flows, webhooks | `github-sync.md` |
| **File formats** — JSON flat / nested / namespaced | `file-formats.md` |
| **Key naming** — conventions, namespaces, anti-patterns | `key-naming.md` |
| **Publish & Analytics** — publish lifecycle, quality checks, CDN analytics | `publish-and-analytics.md` |
...
AI:  [reads mcp.md] Calls getTranslations(status:"missing"), updateKeys, getPendingChanges,
     then publishTranslations — in the correct order with safety checks.
```
